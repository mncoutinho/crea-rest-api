...
import br.org.crea.commons.dao.siacol.SiacolPassivoDao;
...
import br.org.crea.siacol.converter.SiacolPassivoConverter;

public class SiacolRelatorio06BuilderPassivo {
	...
	@Inject	SiacolPassivoDao passivoDao;
	@Inject	SiacolPassivoConverter passivoConverter;
	...
	public List<RelSiacol06Dto> quantidadeDeProtocolosPorAnalistaOuConselheiroPorMes(PesquisaRelatorioSiacolDto pesquisa) {
		...
		for (String mes : mesesDoAno) {
			...
			if (mes.equals("01")) {
				if (passivoDao.existePassivoParaOAno(2019L)) {
					
				    List<RelDetalhadoSiacol06Dto> listaPassivo = passivoConverter.toListDto(passivoDao.getPassivoSemSaidaPorAnoEMes(2019L, 1L));
//					exclui da lista os protocolos que sairam no mês atual - (4)
//					adiciona nessa lista de passivo o que entrou e não saiu no mês e ano vigente(2019) ( 4, 66 )
					if (Integer.parseInt(mes) != DateUtils.getMesCorrente()) {
						List<RelDetalhadoSiacol06Dto> listaPassivoNovoEmJaneiro = new ArrayList<>();
					    for (RelDetalhadoSiacol06Dto novoPassivo : listaPassivoNovoEmJaneiro) {
							SiacolPassivo passivo = new SiacolPassivo();
							
							passivo.setAno(2019L);
							passivo.setMes(Long.parseLong(mes));
							passivo.setProtocoloSiacol(novoPassivo.getNumeroProtocoloLong());
							passivo.setAssuntoSiacol(1L);
							passivo.setUsuario(99990L);
							
							passivoDao.create(passivo);
						} // adiciona na tabela todos os protocolos da lista de passivo do mês/ano vigente(verifica se o item da lista já existe, não faz a operação de inserção)
					}

					List<RelDetalhadoSiacol06Dto> listaPassivoQueEntrouESaiuEmJaneiro = new ArrayList<>();
					for (RelDetalhadoSiacol06Dto naoEhPassivo : listaPassivoQueEntrouESaiuEmJaneiro) {
						passivoDao.deleta(1L);
					} // Exclui da tabela de passivos do mês/ano vigente os protocolos que sairam nesse mesmo mês	( 55 )
						
						
						
				} else if (passivoDao.existePassivoParaOAno(2018L)) {
				    List<RelDetalhadoSiacol06Dto> listaPassivo = passivoConverter.toListDto(passivoDao.getPassivoSemSaidaPorAnoEMes(2018L, null));
				    
				    List<RelDetalhadoSiacol06Dto> listaPassivoQueSaiu = new ArrayList<>();
				    passivoDao.atualizarDataDeSaida(listaPassivoQueSaiu); // exclui da lista de passivo os protocolos que sairam no mês atual ( atualizar na tabela a data de saida ) - 4
	//				adiciona nessa lista de passivo o que entrou e não saiu no mês e ano vigente - 4, 55
					
	
				    List<RelDetalhadoSiacol06Dto> listaPassivoAnoAnteriorMaisJaneiro = new ArrayList<>();
				    for (RelDetalhadoSiacol06Dto novoPassivo : listaPassivoAnoAnteriorMaisJaneiro) { // adiciona na tabela todos os protocolos de passivo do ano vigente e do que restou do ano anterior(lista de passivo), porém, atribuindo o ano atual 2019
						SiacolPassivo passivo = new SiacolPassivo();
	...
	//				adiciona nessa lista de passivos o que entrou e não saiu no mês e ano vigente (2019)

					List<RelDetalhadoSiacol06Dto> listaPassivoAnoAnterior = new ArrayList<>();
					for (RelDetalhadoSiacol06Dto novoPassivo : listaPassivoAnoAnterior) { 	// adiciona todos os protocolos de passivo do ano vigente na tabela, atribuindo o ano atual 2019
						SiacolPassivo passivo = new SiacolPassivo();
			...
			if (!mes.equals("01")) {
				...
				List<RelDetalhadoSiacol06Dto> listaPassivo = passivoConverter.toListDto(passivoDao.getPassivoSemSaidaPorAnoEMes(2019L, 1L)); 				// busca lista de protocolos passivos do mês anterior ( lista em memória do loop do mês anterior )
				
				
				List<RelDetalhadoSiacol06Dto> listaPassivoQueSaiu = new ArrayList<RelDetalhadoSiacol06Dto>();
				
				List<RelDetalhadoSiacol06Dto> listaPassivoQueEntrou = new ArrayList<RelDetalhadoSiacol06Dto>();
				
				passivoDao.atualizarDataDeSaida(listaPassivoQueSaiu); //				exclui da lista de passivos os protocolos que sairam no mês atual ( atualizar na tabela a data de saida dos itens dos meses anteriores )
				
				//				adiciona nessa lista de passivos o que entrou e não saiu no mês vigente - 4, 99
				
				
				if (Integer.parseInt(mes) != DateUtils.getMesCorrente()) {
					for (RelDetalhadoSiacol06Dto novoPassivo : listaPassivoQueEntrou) {
						SiacolPassivo passivo = new SiacolPassivo();
						...
						passivoDao.create(passivo);
					} // adiciona/atualiza na tabela todos os protocolos de passivo do mês/ano vigente(verifica se o item da lista já existe, não faz a operação de inserção)					
				}
			}
			...
			if (Integer.parseInt(mes) == DateUtils.getMesCorrente()) { break; }
		}
	...
	private RelDetalhadoSiacol06Dto getSaidaAnalista(PesquisaRelatorioSiacolDto pesquisa, String mes, RelDetalhadoSiacol06Dto entrada, List<Long> idsAssuntoSiacol) {
		return dao.getSaidaAnalistaDistribuicaoTramitacaoOuAdReferendumOuCriacaoOficio(pesquisa, mes, entrada, idsAssuntoSiacol);
	}
