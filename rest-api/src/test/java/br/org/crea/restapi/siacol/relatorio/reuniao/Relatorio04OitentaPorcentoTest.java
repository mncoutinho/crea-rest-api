...
public class Relatorio04OitentaPorcentoTest {
	...
	public void relatoriosTest() {
		...
		try {
			...
			List<RelatorioReuniaoSiacolDto> listaRelatorio = dao.relatorioDerelatorioDeOitentaPorcento(idReuniao);
			
			listaRelatorio.forEach(item -> {
				System.out.println(item.getNome() + " | " + item.getEntrada() + " | " + item.getSaida()
				+ " | " + item.getTempoPresente() + " | " + item.getHoraOitentaPorcento());
			});
